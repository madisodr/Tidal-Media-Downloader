...
def downloadTrack(track: Track, album=None, playlist=None, userProgress=None, partSize=1048576):
    try:
        ...
        if checkDatabaseForTrack(track, playlist):
            ...
            time.sleep(0.05) # add this sleep step to rate limit how often we hit the tidal API
            return True
        ...
        # i only want to store my playlists for DJing so only add the track to the database if it has a playlist associated with it
